...
export function createFilteredTracesAtom(search: string) {
  const rangeFilteredTracesAtom = atom((get) => {
    const window = get(traceListWindowAtom);
    const traces = get(tracesAtom);
    const loadedOlderIds = search.trim() ? new Set<string>() : get(loadedOlderTraceIdsAtom);
    if (window.mode === "live") {
    ...
    return traces.filter(
      (trace) =>
        loadedOlderIds.has(trace.traceId) || inEventWindow(trace.startEpochNs, fromNs, toNs),
    );
  });

  const searchedTracesAtom = atom((get) => {
    const matches = createTraceSearchMatcher(search.trim());
  ...
  return atom((get) => (search.trim() ? get(searchedTracesAtom) : get(rangeFilteredTracesAtom)));
}

// See rangeFilteredTracesAtom above — same live-tail rolling-window rationale.
export function createFilteredLogsAtom(search: string) {
  const rangeFilteredLogsAtom = atom((get) => {
    const window = get(logListWindowAtom);
    const logs = get(logsAtom);
    const loadedOlderIds = search.trim() ? new Set<string>() : get(loadedOlderLogIdsAtom);
    if (window.mode === "live") {
  ...
  const searchedLogsAtom = atom((get) => {
    const matches = createLogSearchMatcher(search.trim());
    const serverIds = get(serverMatchedLogIdsAtom);
    return get(rangeFilteredLogsAtom).filter((log) => serverIds.has(log.id) || matches(log));
  });

  return atom<LogData[]>((get) =>
    search.trim() ? get(searchedLogsAtom) : get(rangeFilteredLogsAtom),
  );
}

export function createFilteredMetricsAtom(search: string) {
  return atom<MetricData[]>((get) => {
